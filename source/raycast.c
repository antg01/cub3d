...
/*   Updated: 2025/02/12 00:51:21 by gnyssens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void render_3d(t_mlx *data)
{
    for (int x = 0; x < WINDOW_LENGTH; x++) // Loop through each column
    {
        ...
        int mapX = (int)data->player->x_pos; // Current grid cell X
        int mapY = (int)data->player->y_pos; // Current grid cell Y
        ...
        double perpWallDist;
        if (side == 0)
            perpWallDist = (mapX - data->player->x_pos + (1 - stepX) / 2) / rayDirX;
        else
            perpWallDist = (mapY - data->player->y_pos + (1 - stepY) / 2) / rayDirY;
